...
const ChangeRequestDashboard: React.FC = () => {
 ...
 <div className={`inline-flex rounded-full text-sm font-medium border ${getStatusColor(selectedRequest.status)}`}>
 {selectedRequest.status.replace('_', ' ')}
 </div>
 </div>
 <div>
 <span className="text-sm text-gray-500">Priority</span>
 <div className={`inline-flex rounded-full text-sm font-medium border ${getPriorityColor(selectedRequest.priority)}`}>
 ...
 <span className={`inline-flex px-2 rounded-full text-xs font-medium border ${getStatusColor(request.status)}`}>
 {request.status.replace('_', ' ')}
 </span>
 <span className={`inline-flex px-2 rounded-full text-xs font-medium border ${getPriorityColor(request.priority)}`}>
 {request.priority}
 </span>
