...
import { ChatBubbleLeftIcon, PaperAirplaneIcon, UserIcon, PaperClipIcon, XMarkIcon, DocumentIcon, TrashIcon } from '@heroicons/react/24/outline';
...
const TaskComments: React.FC<TaskCommentsProps> = ({ taskId }) => {
  ...
  const addComment = async (content: string, parentId?: string, files?: File[]) => {
    ...
    try {
      ...
      // Use the file upload endpoint if files are present
      if (files && files.length > 0) {
        ...
        const response = await apiClient.post(`/tasks/${taskId}/comments/with-files`, formData, {
          headers: {
        ...
        // Use standard endpoint for text-only comments
        const commentData = {
