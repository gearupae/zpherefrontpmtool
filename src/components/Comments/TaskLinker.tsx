                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center space-x-2 mb-2">
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getLinkTypeColor(link.link_type)}`}>
                        {linkTypes.find(t => t.value === link.link_type)?.label}
                      </span>
                      {getStatusIcon(link.target_task.status)}
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getPriorityColor(link.target_task.priority)}`}>
