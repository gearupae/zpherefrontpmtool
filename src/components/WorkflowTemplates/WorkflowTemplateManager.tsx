...
const WorkflowTemplateManager: React.FC<WorkflowTemplateManagerProps> = ({
 ...
 className="border border-gray-300 rounded-md py-2 text-sm focus:ring-blue-500 focus:border-blue-500"
 >
...
const TemplateCard: React.FC<{
 ...
 <span key={tag} className="text-xs px-2 bg-gray-100 text-gray-600 rounded-full">
 {tag}
 </span>
 ))}
 {template.tags.length > 3 && (
 <span className="text-xs px-2 bg-gray-100 text-gray-600 rounded-full">
 ...
 <button
 onClick={onUse}
 className="bg-blue-600 text-white text-sm rounded-md hover:bg-blue-700"
 >
...
const TemplateListItem: React.FC<{
 ...
 <span className={`text-xs px-2 rounded-full flex-shrink-0 ${
 template.complexity_level === 'low' ? 'bg-green-100 text-green-700' :
 template.complexity_level === 'medium' ? 'bg-yellow-100 text-yellow-700' :
 'bg-red-100 text-red-700'
 }`}>
