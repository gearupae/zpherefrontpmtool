...
const fetchContextCards = async () => {
  try {
    ...
    const normalizedCards = cards.map(card => ({
      ...
      linked_tasks: Array.isArray(card.linked_tasks) ? card.linked_tasks : [],
      linked_projects: Array.isArray(card.linked_projects) ? card.linked_projects : [],
      linked_discussions: Array.isArray(card.linked_discussions) ? card.linked_discussions : [],
    }));
    setContextCards(normalizedCards);
  } catch (error) {
 ...
 decision_rationale: formData.decision_rationale || undefined,
 ...
 const normalize = (card: any): ContextCard => ({
   ...card,
   tags: Array.isArray(card?.tags) ? card.tags : [],
   linked_tasks: Array.isArray(card?.linked_tasks) ? card.linked_tasks : [],
   linked_projects: Array.isArray(card?.linked_projects) ? card.linked_projects : [],
 ...
 if (editingCard) {
   const res = await apiClient.put(`/context-cards/${editingCard.id}`, payload);
   const updated = normalize(res.data);
   setContextCards(prev => prev.map(c => c.id === updated.id ? updated : c));
 } else {
   const res = await apiClient.post('/context-cards/', payload);
   const created = normalize(res.data);
   setContextCards(prev => [created, ...prev]);
 }
 resetForm();
 } catch (error) {
 ...
 setContextCards(prev => prev.filter(c => c.id !== id));
 } catch (error) {
 ...
 setContextCards(prev => prev.map(c => c.id === id ? { ...c, is_archived: archive } : c));
 } catch (error) {
 ...
 className="w-full bg-white border-2 border-dashed border-gray-300 rounded-lg p-2 text-black hover:bg-gray-50 flex items-center justify-center gap-2"
 >
 ...
 View all {filteredCards.length} context cards
          ...
          {/* Inline Create Form (compact) */}
          {showForm && (
            <form onSubmit={handleSubmit} className="bg-white border rounded-lg p-3 space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <div>
                  ...
                  <input
                    ...
                    className="w-full py-1.5 px-2 border border-gray-300 rounded-md focus:ring-black focus:border-black text-sm"
                  />
                </div>
                <div>
              ...
              <div className="flex justify-end gap-2">
                ...
                <button
                  type="submit"
                  className="px-3 py-1.5 bg-black text-white rounded-md hover:bg-black/90 text-sm"
                >
                  Save
                </button>
              </div>
 ...
 <span className="ml-2 inline-flex px-2 text-xs rounded-full bg-gray-100 text-gray-800">
 ...
 <span key={taskId} className="inline-flex px-2 text-xs rounded bg-gray-100 text-gray-800">
 Task {taskId}
 </span>
 ))}
 </div>
 ...
 <span key={projectId} className="inline-flex px-2 text-xs rounded bg-gray-100 text-gray-800">
 Project {projectId}
 </span>
 ))}
 </div>
 ...
 <span key={discussionId} className="inline-flex px-2 text-xs rounded bg-gray-100 text-gray-800">
 ...
 <button
 onClick={() => setShowForm(true)}
 className="bg-black text-white px-4 py-2 rounded-md hover:bg-black/90 flex items-center gap-2"
 >
 ...
 className="py-2 border border-gray-300 rounded-md focus:ring-black focus:border-black"
 >
 ...
 {card.auto_captured && (
 <span className="inline-flex px-2 text-xs rounded-full bg-gray-100 text-gray-800">
 Auto
 </span>
 )}
 </div>
 ...
 <button
 onClick={() => setShowForm(true)}
 className="mt-4 bg-black text-white px-4 py-2 rounded-md hover:bg-black/90"
 >
 ...
 {getTypeIcon(selectedCard.context_type)}
 <h3 className="text-xl font-semibold text-gray-900">{selectedCard.title}</h3>
 <span className={`inline-flex px-2 text-xs font-semibold rounded-full ${getTypeColor(selectedCard.context_type)}`}>
 {selectedCard.context_type}
 </span>
 <span className={`inline-flex px-2 text-xs font-semibold rounded-full ${getPriorityColor(selectedCard.priority)}`}>
 {selectedCard.priority}
 </span>
 </div>
 <div className="text-sm text-gray-500">
 Created by {selectedCard.created_by_name} on {new Date(selectedCard.created_at).toLocaleDateString()}
 {selectedCard.auto_captured && (
 <span className="ml-2 inline-flex px-2 text-xs rounded-full bg-green-100 text-green-800">
 Auto-captured from {selectedCard.capture_source}
 </span>
 )}
 </div>
 ...
 <div className="flex flex-wrap gap-1 mt-1">
 {selectedCard.linked_tasks.map(taskId => (
 <span key={taskId} className="inline-flex px-2 text-xs rounded bg-blue-100 text-blue-700">
 Task {taskId}
 </span>
 ))}
 </div>
 ...
 <div className="flex flex-wrap gap-1 mt-1">
 {selectedCard.linked_projects.map(projectId => (
 <span key={projectId} className="inline-flex px-2 text-xs rounded bg-green-100 text-green-700">
 Project {projectId}
 </span>
 ))}
 </div>
 ...
 <div className="flex flex-wrap gap-1 mt-1">
 {selectedCard.linked_discussions.map(discussionId => (
 <span key={discussionId} className="inline-flex px-2 text-xs rounded bg-purple-100 text-purple-700">
 Discussion {discussionId}
 </span>
 ))}
 </div>
