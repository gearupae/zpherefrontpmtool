...
const KnowledgeHubPage: React.FC = () => {
 ...
 // Ensure API responses are normalized to arrays
 ...
 setArticles(normalizeArray<KnowledgeArticle>(articlesResponse?.data));
 break;
 case 'context':
 const contextResponse = await apiClient.get('/context-cards/');
 setContextCards(normalizeArray<ContextCard>(contextResponse?.data));
 break;
 case 'decisions':
 const decisionsResponse = await apiClient.get('/decision-logs/');
 setDecisions(normalizeArray<DecisionLog>(decisionsResponse?.data));
 break;
 }
 } catch (error) {
 console.error('Error fetching data:', error);
 // Ensure state remains arrays even on error
 if (activeTab === 'articles') setArticles([]);
 if (activeTab === 'context') setContextCards([]);
 if (activeTab === 'decisions') setDecisions([]);
 } finally {
 setLoading(false);
 }
 }, [activeTab]);
 ...
 const filteredContextCards = (Array.isArray(contextCards) ? contextCards : []).filter(card =>
 (card.title || '').toLowerCase().includes(search) ||
 (card.content || '').toLowerCase().includes(search) ||
 (filterType === '' || (card.context_type || '') === filterType)
 );

 const filteredDecisions = (Array.isArray(decisions) ? decisions : []).filter(decision =>
 (decision.title || '').toLowerCase().includes(search) ||
 (decision.description || '').toLowerCase().includes(search) ||
 ...
 className={`whitespace-nowrap py-2 font-medium text-sm rounded-md transition-colors focus:outline-none focus:ring-0 ${
 ...
 <span key={index} className="inline-flex items-center px-2 rounded-md text-xs font-medium bg-blue-100 text-blue-800">
 {tag}
 </span>
 ))}
 </div>
 ...
 className="px-3.5 text-sm bg-black text-white rounded hover:bg-gray-800"
 >
 View
 </button>
 </div>
...
const ArticleDetailsModal: React.FC<{ articleId: string; initialArticle?: any; onClose: () => void }> = ({ articleId, initialArticle, onClose }) => {
 ...
 {loading ? (
 <div className="flex items-center justify-center6">
 ...
 <span key={idx} className="inline-flex items-center px-2 rounded-md text-xs font-medium bg-blue-100 text-blue-800">{t}</span>
 ))}
 </div>
 </div>
 )}
