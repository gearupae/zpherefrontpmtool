...
const TaskDetailOverviewPage: React.FC = () => {
 ...
 const { members, setMode, sendTyping, lastTyping, lastComment } = useTaskPresence(id, 'viewing');
 ...
 // Helper to normalize time tracking metadata
 const getTimeTracking = (t: Task | null) => {
 const meta = ((t?.metadata || {}) as any);
 const tt = (meta?.time_tracking || {}) as any;
 return {
 running: !!tt.running,
 current_start: tt.current_start || null as string | null,
 last_stop: tt.last_stop || null as string | null,
 total_seconds: typeof tt.total_seconds === 'number' ? tt.total_seconds : 0,
 history: Array.isArray(tt.history) ? tt.history as Array<{ start: string; stop?: string; duration_seconds?: number; user_id?: string; }> : []
 };
 };

 // Tick every second while timing is running to update UI
 useEffect(() => {
 const running = getTimeTracking(task).running;
 if (!running) return;
 const i = setInterval(() => setNowTs(Date.now()), 1000);
 return () => clearInterval(i);
 // eslint-disable-next-line react-hooks/exhaustive-deps
 }, [task?.id, (task as any)?.metadata?.time_tracking?.running]);

 const handleStartStopTiming = async () => {
 if (!task) return;
 try {
 setIsSavingTime(true);
 const { default: apiClient } = await import('../../api/client');
 const tt = getTimeTracking(task);
 ...
 ...tt,
 running: true,
 current_start: nowIso,
 };
 const newMetadata = { ...(task.metadata || {}), time_tracking: newTT } as any;
 const payload: any = { metadata: newMetadata };
 if (task.status === TaskStatus.TODO) {
 payload.status = TaskStatus.IN_PROGRESS;
 }
 const res = await apiClient.put(`/tasks/${task.id}/`, payload);

 const updatedTask = res.data as Task;
 setTask(updatedTask);
 calculateStats(updatedTask);
 dispatch(addNotification({ type: 'success', title: 'Timer Started', message: 'Task timer started.', duration: 2000 }));
 } else {
 // Stop timing
 const startMs = tt.current_start ? new Date(tt.current_start).getTime() : null;
 const deltaSec = startMs ? Math.max(0, Math.floor((Date.now() - startMs) / 1000)) : 0;
 const newTotal = (tt.total_seconds || 0) + deltaSec;
 const newHistory = [...tt.history, { start: tt.current_start as string, stop: nowIso, duration_seconds: deltaSec, user_id: currentUser?.id }];
 const newTT = {
 running: false,
 current_start: null,
 last_stop: nowIso,
 ...
 const newActualHours = parseFloat((newTotal / 3600).toFixed(2));
 const newMetadata = { ...(task.metadata || {}), time_tracking: newTT } as any;
 const payload: any = { metadata: newMetadata, actual_hours: newActualHours };
 const res = await apiClient.put(`/tasks/${task.id}`, payload);
 const updatedTask = res.data as Task;
 setTask(updatedTask);
 calculateStats(updatedTask);
 dispatch(addNotification({ type: 'success', title: 'Timer Stopped', message: 'Task timer stopped and saved.', duration: 2000 }));
 }
 } catch (e) {
 console.error('Failed to toggle time tracking', e);
 dispatch(addNotification({ type: 'error', title: 'Time Tracking Failed', message: 'Could not update task timer.', duration: 4000 }));
 } finally {
 ...
 // Update presence mode based on active tab
 useEffect(() => {
 try { setMode(activeTab === 'edit' ? 'editing' : 'viewing'); } catch {}
 }, [activeTab, setMode]);
 ...
 // Subtasks API functions
 const fetchSubtasks = async () => {
 if (!id) return;
 setIsLoadingSubtasks(true);
 try {
 const { default: apiClient } = await import('../../api/client');
 const res = await apiClient.get('/tasks/', { params: { parent_task_id: id } });
 ...
 setNewSubtaskTitle('');
 dispatch(addNotification({ type: 'success', title: 'Subtask Created', message: 'Subtask has been created.', duration: 2500 }));
 fetchActivities();
 } catch (e) {
 console.error('Failed to create subtask', e);
 dispatch(addNotification({ type: 'error', title: 'Failed to create subtask', message: 'There was an error creating the subtask.', duration: 4000 }));
 }
 };
 ...
 // Derive time tracking display values
 const timeTracking = getTimeTracking(task);
 const isRunning = !!timeTracking.running;
 const startMs = timeTracking.current_start ? new Date(timeTracking.current_start).getTime() : null;
 const baseTotalSec = timeTracking.total_seconds || 0;
 const runningElapsedSec = isRunning && startMs ? Math.max(0, Math.floor((nowTs - startMs) / 1000)) : 0;
 const displayTotalSec = baseTotalSec + runningElapsedSec;
 const displayTotalHours = displayTotalSec / 3600;
 const lastHistory = timeTracking.history.length > 0 ? timeTracking.history[timeTracking.history.length - 1] : null;
 const startedDisplayISO = isRunning ? timeTracking.current_start : (lastHistory ? lastHistory.start : null);
 const stoppedDisplayISO = !isRunning ? (timeTracking.last_stop || (lastHistory ? lastHistory.stop : null)) : null;
 ...
 {/* Live indicators */}
 {(lastTyping || lastComment) && (
 <div className="mt-1 text-xs">
 {lastTyping && (
 <div className="text-gray-600">
 Live: @{(members.find(m => m.userId === lastTyping.userId)?.user?.name) || lastTyping.userId} typing {lastTyping.field}
 {lastTyping.preview ? (
 <>
 :
 <span className="ml-1 italic text-gray-700">“{String(lastTyping.preview).slice(0, 80)}”</span>
 </>
 ) : null}
 </div>
 )}
 {lastComment && (
 <div className="text-gray-600">
 💬 @{lastComment.authorName || lastComment.authorId || 'someone'} left a comment just now
 </div>
 )}
 </div>
 )}
 ...
 } ${isSavingTime ? 'opacity-60 cursor-not-allowed' : ''}`}
 >
 {isRunning ? (
 ...
 disabled={!project}
 className="flex items-center space-x-2 rounded-md text-sm font-medium transition-colors shadow-sm border bg-white text-gray-900 border-gray-200 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
 >
 ...
 {/* Start/Stop timestamps and total hours */}
 <div className="grid grid-cols-3 gap-4 text-center mt-4">
 <div>
 <div className="text-xs text-gray-500">Started</div>
 <div className="text-sm font-medium text-gray-900">{formatDateTime(startedDisplayISO)}</div>
 </div>
 <div>
 <div className="text-xs text-gray-500">Stopped</div>
 <div className="text-sm font-medium text-gray-900">{formatDateTime(stoppedDisplayISO)}</div>
 </div>
 <div>
 <div className="text-xs text-gray-500">Total Hours</div>
 <div className="text-2xl font-bold text-green-700">{displayTotalHours.toFixed(2)}h</div>
 </div>
 </div>
 ...
 <div className="text-sm text-gray-500">Time Spent (saved)</div>
 </div>
 <div>
 ...
 rows={3}
 value={newComment}
 onChange={(e) => { setNewComment(e.target.value); try { sendTyping('comment', e.target.value); } catch {} }}
 ...
 className="w-full border border-gray-300 rounded-md focus:ring-2 focus:ring-primary-500 focus:border-transparent py-2"
 ...
 <div className="mt-1 text-xs text-gray-500">{formatRelativeTime(activity.created_at)}</div>
 </div>
 </div>
 </div>
 </li>
 ...
 className="w-64 border border-gray-300 rounded-md py-2 focus:ring-2 focus:ring-primary-500 focus:border-transparent"
 />
 <button onClick={handleCreateSubtask} disabled={!newSubtaskTitle.trim()} className="btn btn-primary">
 ...
 {st.status.replace('_', ' ')} • {st.priority}
 </div>
 </div>
 <div className="flex items-center gap-2">
 <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(st.status)}`}>{st.status.replace('_', ' ')}</span>
 </div>
 </li>
 ))}
 </ul>
 ...
 onChange={(e) => { setEditForm({ ...editForm, title: e.target.value }); try { sendTyping('title', e.target.value); } catch {} }}
 ...
 onChange={(e) => { setEditForm({ ...editForm, status: e.target.value as TaskStatus }); try { sendTyping('status', e.target.value); } catch {} }}
 className="w-full py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-primary-500 focus:border-transparent"
 >
 ...
 onChange={(e) => { const v = e.target.value; setEditForm({ ...editForm, estimated_hours: parseFloat(v) || 0 }); try { sendTyping('estimated_hours', v); } catch {} }}
 className="w-full py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-primary-500 focus:border-transparent"
 />
 </div>
 ...
 <li
 key={user.id}
 className={`py-2 text-sm hover:bg-gray-50 ${editForm.created_by_id === user.id ? 'bg-gray-50' : ''}`}
 ...
 onChange={(e) => { setEditForm({ ...editForm, description: e.target.value }); try { sendTyping('description', e.target.value); } catch {} }}
 className="w-full py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-primary-500 focus:border-transparent"
 ...
 <label className="block text-sm font-medium text-gray-700 mb-2">Visible to customer</label>
 <VisibleToCustomerRadio
 value={(editForm as any).visible_to_customer}
 onChange={(val) => setEditForm(prev => ({ ...prev, visible_to_customer: val }))}
 helperText="If enabled, customers can view this task."
 />
 </div>
