...
const AnalyticsPage: React.FC = () => {
 ...
 if (!safeProjects || !safeTasks || !safeTeamMembers) return null;
 ...
 const overdueProjects = safeProjects.filter((p: any) => {
 ...
 const pendingTasks = safeTasks.filter((t: any) => t.status === 'todo').length;
 const overdueTasks = safeTasks.filter((t: any) => {
 ...
 const totalTeamMembers = safeTeamMembers.length;
 const activeMembers = safeTeamMembers.filter((m: any) => m.is_active).length;

 // Project status distribution
 const projectStatusData = {
 'Active': activeProjects,
 'Completed': completedProjects,
'Planning': safeProjects.filter((p: any) => p.status === 'planning').length,
 'On Hold': safeProjects.filter((p: any) => p.status === 'on_hold').length,
 'Cancelled': safeProjects.filter((p: any) => p.status === 'cancelled').length,
 };
 ...
 'Todo': pendingTasks,
'In Progress': safeTasks.filter((t: any) => t.status === 'in_progress').length,
 'Overdue': overdueTasks,
 };

 // Priority distribution
 const priorityData = {
'High': safeTasks.filter((t: any) => t.priority === 'high').length,
 'Medium': safeTasks.filter((t: any) => t.priority === 'medium').length,
 'Low': safeTasks.filter((t: any) => t.priority === 'low').length,
 };
 ...
 ? safeProjects.find((p: any) => p.id === selectedProject)
 : null;
 ...
 className="inline-flex items-center py-2 border border-gray-300 text-sm font-medium rounded-md text-[#0d0d0d] bg-white hover:bg-blue-50 hover:text-blue-700 hover:border-blue-300 transition-colors"
 >
 ...
 <div className={`inline-flex items-center rounded-full text-sm font-medium ${getPriorityColor(priority)} mb-2`}>
 {priority} Priority
 </div>
 ...
 {activeTab === 'team' && (
 <TeamPerformance teamMembers={safeTeamMembers} tasks={safeTasks} />
 )}
 ...
 className="flex-1 border border-gray-300 rounded-l-md py-2 bg-gray-50 text-sm"
 />
 <button
 onClick={() => navigator.clipboard.writeText('https://zphere.com/shared/analytics/abc123')}
 className="py-2 border border-l-0 border-gray-300 rounded-r-md bg-gray-50 hover:bg-gray-100 text-sm"
 >
 Copy
 </button>
 </div>
