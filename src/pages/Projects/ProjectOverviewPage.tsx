...
const ProjectOverviewPage: React.FC = () => {
 console.log('[ProjectOverviewPage] 🚀 Component mounting/rendering at:', window.location.pathname);
...
// Table sorting state
 ...
 useEffect(() => {
 console.log('[ProjectOverviewPage] useEffect[1]: Initial data fetch');
 ...
 useEffect(() => {
 console.log('[ProjectOverviewPage] useEffect[2]: Path changed to:', location.pathname);
 dispatch(fetchProjects({}));
 fetchCustomers();
 ...
 console.log('[ProjectOverviewPage] fetchProjectsData: Starting for', projects.length, 'projects');
...
const tasksEndpoint = `/tasks?project_id=${project.id}`;
 ...
 console.error(`❌ [ProjectOverviewPage] Failed to fetch data for project ${project.id}:`, projectError);
 console.error(' Error detail:', projectError?.response?.data?.detail || projectError?.message);
 ...
 setProjectData(data);
 console.log('[ProjectOverviewPage] fetchProjectsData: ✅ Success');
 } catch (error: any) {
 console.error('❌ [ProjectOverviewPage] Failed to fetch project data:', error);
 ...
 try {
 console.log('[ProjectOverviewPage] fetchCustomers: Starting');
 const { default: apiClient } = await import('../../api/client');
...
// Always use tenant customers endpoint
 ...
 setCustomers(list || []);
 console.log('[ProjectOverviewPage] fetchCustomers: ✅ Success,', list.length, 'customers');
 } catch (error: any) {
 console.error('❌ [ProjectOverviewPage] Failed to fetch customers:', error);
 console.error(' Error detail:', error?.response?.data?.detail || error?.message);
 setCustomers([]);
 }
 };
 ...
 try {
 console.log('[ProjectOverviewPage] fetchAvailableUsers: Starting');
 const { default: apiClient } = await import('../../api/client');
 ...
 console.log('[ProjectOverviewPage] fetchAvailableUsers: ✅ Success,', response.data?.length || 0, 'users');
 } catch (error: any) {
 console.error('❌ [ProjectOverviewPage] Failed to fetch team members:', error);
 console.error(' Error detail:', error?.response?.data?.detail || error?.message);
 setAvailableUsers([]);
 }
 };
...
const handleCreateProject = async (e: React.FormEvent) => {
 ...
 className={`flex items-center space-x-2 rounded-md text-sm font-medium transition-colors shadow-sm border ${viewMode === 'cards' ? 'bg-black text-white border-black' : 'bg-white text-gray-900 border-gray-200 hover:bg-gray-50'}`}
 ...
 className={`flex items-center space-x-2 rounded-md text-sm font-medium transition-colors shadow-sm border ${viewMode === 'table' ? 'bg-black text-white border-black' : 'bg-white text-gray-900 border-gray-200 hover:bg-gray-50'}`}
 ...
 className={`flex items-center space-x-2 rounded-md text-sm font-medium transition-colors shadow-sm border ${viewMode === 'kanban' ? 'bg-black text-white border-black' : 'bg-white text-gray-900 border-gray-200 hover:bg-gray-50'}`}
 ...
 className={`flex items-center space-x-2 rounded-md text-sm font-medium transition-colors shadow-sm border ${viewMode === 'calendar' ? 'bg-black text-white border-black' : 'bg-white text-gray-900 border-gray-200 hover:bg-gray-50'}`}
 ...
 className={`flex items-center space-x-2 rounded-md text-sm font-medium transition-colors shadow-sm border ${viewMode === 'gantt' ? 'bg-black text-white border-black' : 'bg-white text-gray-900 border-gray-200 hover:bg-gray-50'}`}
...
<button className="filter-popup-btn filter-popup-btn-filter" onClick={(e) => {
 ...
 <tr>
 <th onDoubleClick={() => onHeaderDblClick('name')} className="py-2 text-left text-sm font-semibold text-black uppercase tracking-wider cursor-pointer select-none" title="Double‑click to sort by Project Name">
 ...
 <th onDoubleClick={() => onHeaderDblClick('priority')} className="py-2 text-left text-sm font-semibold text-black uppercase tracking-wider cursor-pointer select-none">
 <div className="inline-flex items-center gap-1">
 <span>Priority</span>
 <span className="relative">
 <button
 ...
 <th onDoubleClick={() => onHeaderDblClick('start_date')} className="py-2 text-left text-sm font-semibold text-black uppercase tracking-wider cursor-pointer select-none">
 <div className="inline-flex items-center gap-1">
 ...
 <th onDoubleClick={() => onHeaderDblClick('due_date')} className="py-2 text-left text-sm font-semibold text-black uppercase tracking-wider cursor-pointer select-none">
 <div className="inline-flex items-center gap-1">
 <span>Due Date</span>
 <span className="relative">
 <button
 ...
 <th onDoubleClick={() => onHeaderDblClick('tasks')} className="py-2 text-left text-sm font-semibold text-black uppercase tracking-wider cursor-pointer select-none" title="Double‑click to sort by Tasks">
 <div className="inline-flex items-center gap-1">
 <span>Tasks</span>
 {sortField === 'tasks' && (
 sortDir === 'asc' ? (
 ...
 <th onDoubleClick={() => onHeaderDblClick('time_log')} className="py-2 text-left text-sm font-semibold text-black uppercase tracking-wider cursor-pointer select-none">
 Time Log
 </th>
 <th onDoubleClick={() => onHeaderDblClick('completion')} className="py-2 text-left text-sm font-semibold text-black uppercase tracking-wider cursor-pointer select-none">
 Completion
 </th>
 <th onDoubleClick={() => onHeaderDblClick('budget')} className="py-2 text-left text-sm font-semibold text-black uppercase tracking-wider cursor-pointer select-none">
 Budget
 </th>
 <th className="py-2 text-left text-sm font-semibold text-black uppercase tracking-wider">
 Actions
 ...
 <span className="text-gray-500 mr-2">P-{index + 1}</span>
 {project.name}
 </div>
 </div>
 </div>
...
<td className="py-2 whitespace-nowrap" onClick={(e) => { e.stopPropagation(); startInlineEdit(e, project, 'status'); }}>
...
<span className={`inline-flex px-1.5 pt-0 pb-0.5 text-xs font-light rounded-sm ${getStatusColorBold(project.status)}`}>
 ...
 <li
 key={opt}
 className={`px-2 rounded-sm hover:bg-gray-50 cursor-pointer flex items-center justify-between ${opt===project.status ? 'bg-gray-50' : ''}`}
...
<td className="py-2 whitespace-nowrap" onClick={(e) => { e.stopPropagation(); startInlineEdit(e, project, 'priority'); }}>
 <div className="relative inline-block">
<span className={`inline-flex px-1.5 pt-0 pb-0.5 text-xs font-light rounded-sm ${getPriorityColorBold(project.priority)}`}>
 ...
 <li
 key={opt}
 className={`px-2 rounded-sm hover:bg-gray-50 cursor-pointer flex items-center justify-between ${opt===project.priority ? 'bg-gray-50' : ''}`}
 onClick={() => { saveInlineEdit(project.id, opt); }}
 >
 ...
 {/* Team Members */}
<td className="py-2 whitespace-nowrap cursor-pointer select-none" onClick={(e) => openTeamInlineEditor(e, project.id)}>
 ...
 {/* Start Date */}
 <td className="py-2 whitespace-nowrap text-sm text-gray-500" onClick={(e) => { e.stopPropagation(); startInlineEdit(e, project, 'start_date'); }}>
 <div className="relative inline-block">
 {project.start_date ? (
 ...
 {/* Due Date */}
 <td className="py-2 whitespace-nowrap text-sm text-gray-500" onClick={(e) => { e.stopPropagation(); startInlineEdit(e, project, 'due_date'); }}>
 <div className="relative inline-block">
 {project.due_date ? (
...
className="inline-flex items-center justify-center p-2 rounded-md bg-gray-100 text-black hover:bg-gray-200 transition-colors"
 ...
 <div className="px-1.5 text-xs text-gray-800 font-medium">Filter team</div>
 <ul className="max-h-48 overflow-auto">
 ...
 <FilterPortal buttonRect={filterButtonRect}>
 <div ref={headerFilterRef} className="w-52 border border-gray-200 bg-white shadow-sm p-1.5 text-xs font-medium" style={{borderRadius: '5px'}}>
 <div className="px-1.5 text-xs text-gray-800 font-medium">Filter client</div>
 <ul className="max-h-48 overflow-auto">
 {customers.map(c => {
 ...
 <button
 className="px-3.5 text-sm rounded-md bg-gray-100 text-gray-800 hover:bg-gray-200"
 onClick={() => setConfirmDeleteId(null)}
 >
 Cancel
 </button>
 <button
 className="px-3.5 text-sm rounded-md bg-red-600 text-white hover:bg-red-700"
 onClick={async () => {
