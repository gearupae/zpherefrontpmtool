...
const GoalDetailOverviewPage: React.FC = () => {
 ...
 <h2 className="text-2xl font-bold text-gray-900 mb-4">Goal Not Found</h2>
            <button onClick={() => navigate('/goals')} className="bg-blue-600 text-white rounded-md">Back to Goals</button>
 </div>
 );
 }
 ...
 goal.completion_percentage >= 50 ? 'bg-yellow-500' : 'bg-blue-600'
 }`}></div>
 ...
 <h3 className="text-lg font-medium text-gray-900">Team Members</h3>
                <button onClick={() => setIsTeamDropdownOpen(true)} className="inline-flex items-center text-sm rounded-md text-white bg-blue-600 hover:bg-blue-700">
 ...
 .map((user) => (
 <li key={user.id} className="py-2 text-sm flex items-center justify-between hover:bg-gray-50">
 <div className="flex flex-col">
 <span className="text-gray-900">{user.full_name || `${user.first_name || ''} ${user.last_name || ''}`.trim()}</span>
 <span className="text-xs text-gray-500">{user.email}</span>
 </div>
 <div className="flex space-x-2">
 <button onClick={() => { handleAddMember(user.id, 'member'); setIsTeamDropdownOpen(false); }} className="px-2 text-xs font-medium text-blue-700 bg-blue-100 rounded-md hover:bg-blue-200">Add as Member</button>
 <button onClick={() => { handleAddMember(user.id, 'owner'); setIsTeamDropdownOpen(false); }} className="px-2 text-xs font-medium text-green-700 bg-green-100 rounded-md hover:bg-green-200">Add as Owner</button>
 <button onClick={() => { handleAddMember(user.id, 'viewer'); setIsTeamDropdownOpen(false); }} className="px-2 text-xs font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200">Add as Viewer</button>
 </div>
 </li>
 ))}
 {availableUsers.filter(u => !teamMembers.find(tm => tm.id === u.id)).length === 0 && (
 <li className="py-2 text-sm text-gray-500">No users available to add</li>
 )}
 </ul>
 </div>
 </div>
...
const AddChecklistInline: React.FC<{ goalId: string; onAdded: () => void }> = ({ goalId, onAdded }) => {
 ...
 className="flex-1 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
 />
 {/* Due date */}
 <div className="relative">
 <button
 type="button"
 className="px-2 border border-gray-300 rounded-md text-sm text-gray-700 bg-white hover:bg-gray-50"
 ...
 <input type="date" value={due} onChange={(e) => setDue(e.target.value)} className="text-sm border border-gray-300 rounded px-2" />
 </div>
 )}
 </div>
 {/* Priority */}
 ...
 <button
 type="button"
 className="px-2 border border-gray-300 rounded-md text-sm text-gray-700 bg-white hover:bg-gray-50 capitalize"
 ...
 <div
 key={p}
 className={`text-sm cursor-pointer hover:bg-gray-50 ${p===priority ? 'bg-gray-50' : ''}`}
 ...
 <button
 type="button"
 className="py-2 bg-black text-white rounded-md text-sm"
 onClick={submit}
 >
